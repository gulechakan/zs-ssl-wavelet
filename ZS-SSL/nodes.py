...
def cyclic_conv1d_alt(input_node, coeffs, filter_):
    """
...
def dwt1d(input_node, d4_coeffs_decomp_lp, d4_coeffs_decomp_hp, wavelet, levels=1):
    """
    ...
    for level in range(levels):
        lp_res = cyclic_conv1d_alt(last_level, d4_coeffs_decomp_lp, wavelet.decomp_lp)[:, ::2, :]
        hp_res = cyclic_conv1d_alt(last_level, d4_coeffs_decomp_hp, wavelet.decomp_hp)[:, 1::2, :]
...
def dwt2d(input_node, d4_coeffs_decomp_lp, d4_coeffs_decomp_hp, wavelet, levels=1):
    """
    ...
    for level in range(levels):
        ...
        first_pass = dwt1d(last_level, d4_coeffs_decomp_lp, d4_coeffs_decomp_hp, wavelet, 1)
        second_pass = tf.transpose(
            dwt1d(
                tf.transpose(first_pass, perm=[1, 0, 2]),
                d4_coeffs_decomp_lp, d4_coeffs_decomp_hp, wavelet,
                1
            ),
            perm=[1, 0, 2]
        )
    ...
    last_level = tf.transpose(
        dwt1d(
            tf.transpose(last_level, perm=[1, 0, 2]),
            d4_coeffs_decomp_lp, d4_coeffs_decomp_hp, wavelet,
            1
        ),
        perm=[1, 0, 2]
...
def idwt1d(input_node, d4_coeffs_recon_lp, d4_coeffs_recon_hp, wavelet, levels=1):
    """
    ...
    for level in range(levels - 1, -1 , -1):
        ...
        lowres_filtered = cyclic_conv1d_alt(lowres_padded, d4_coeffs_recon_lp, wavelet.recon_lp)
        detail_filtered = cyclic_conv1d_alt(detail_padded, d4_coeffs_recon_hp, wavelet.recon_hp)
...
def idwt2d(input_node, d4_coeffs_recon_lp, d4_coeffs_recon_hp, wavelet, levels=1):
    """
    ...
    last_level = tf.transpose(
        idwt1d(
            tf.transpose(last_level, perm=[1, 0, 2]), d4_coeffs_recon_lp, d4_coeffs_recon_hp,
            wavelet,
            1
        ),
        perm=[1, 0, 2]
    ...
    for level in range(levels - 1, -1, -1):
        ...
        first_pass = tf.transpose(
            idwt1d(
                tf.transpose(this_level, perm=[1, 0, 2]), d4_coeffs_recon_lp, d4_coeffs_recon_hp,
                wavelet,
                1
            ),
            perm=[1, 0, 2]
        ...
        second_pass = idwt1d(first_pass, d4_coeffs_recon_lp, d4_coeffs_recon_hp, wavelet, 1)
